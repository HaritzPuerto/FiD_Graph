...
def format_data(tokenizer, data, url2doc, sent_sep_encoding, max_length: 512, print_info: False):
    ...
    for x in tqdm(data):
        ...
        for s in list_sections:
            truncated_sec_txt, list_sent_idx = truncate_section(tokenizer, s, max_length, sent_sep_encoding)
...
if __name__ == "__main__":
    ...
    with open("../data/fid_format/train.json", 'w') as f:
    ...
    with open("../data/fid_format/dev.json", 'w') as f:
        json.dump(dev_dataset, f)
        
    with open("../data/fid_format/DONE.txt", 'w') as f:
        f.write("DONE")